...
def export_sweep_pptx(
    ...
    """
    Generate a sweep summary PPTX from a list of per-run results.
    ...
    work_dir = tempfile.mkdtemp(prefix="mlp_sweep_pptx_")
    try:
        # --- summary figure (slide 1) ---
        summary_path = os.path.join(work_dir, "fig_summary.png")
        if verbose:
            print("mlp_to_pptx: generating sweep summary figure …")
        _make_summary_figure(sweep_results, summary_path)
        ...
        for i, entry in enumerate(sweep_results):
            ...
            f1 = os.path.join(work_dir, f"run{i:02d}_fig1.png")
            f2 = os.path.join(work_dir, f"run{i:02d}_fig2.png")
            f3 = os.path.join(work_dir, f"run{i:02d}_fig3.png") \
                 if transfer_results is not None else None

            _make_distribution_figure(mlp_results, f1)
            _make_results_figure(mlp_results, f2, top_n=top_n)
            if transfer_results is not None:
                _make_transfer_figure(transfer_results, f3)

            run_meta = _build_meta(
                mlp_results, hp, f"{case_name} — {run_name}",
                f1, f2, f3, pptx_out,
                transfer_results=transfer_results,
            )
        ...
        # --- build sweep summary stats for JS ---
        sweep_summary = []
        for i, entry in enumerate(sweep_results):
            run_name, mlp_results, transfer_results, hp = entry
            eps = 1e-12
            tr_r = None
            if transfer_results is not None:
                ly_p = np.log(np.maximum(transfer_results['y_pred'], eps))
                ly_t = np.log(np.maximum(transfer_results['y_targ'], eps))
                tr_r, _ = stats.pearsonr(ly_t, ly_p)
            sweep_summary.append({
                "run_name": run_name,
                "train_r2": float(mlp_results['r2_train']),
                "test_r2":  float(mlp_results['r2_test']),
                "test_r":   float(mlp_results['r_test']),
                "transfer_r": float(tr_r) if tr_r is not None else None,
            })
        ...
        if verbose:
            print("mlp_to_pptx: assembling sweep deck …")
        result = subprocess.run(
            ["node", js_script, meta_path],
            capture_output=True, text=True,
        )
...
# ---------------------------------------------------------------------------
# Figure 0 — Sweep summary bar chart
# ---------------------------------------------------------------------------

def _make_summary_figure(sweep_results, out_path):
